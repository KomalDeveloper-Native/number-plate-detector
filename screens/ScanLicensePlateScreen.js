  ...
  const capturePhoto = async () => {
    if (camera.current && !isProcessing) {
      ...
      try {
        ...
        const fullText = result.text.trim().replace(/\s+/g, '');
        console.log('Image Path:', imagePath);
        console.log('Recognized Text:', result.text);
        const licensePlate =
          fullText.match(/[A-Z]{2}[0-9]{2}[A-Z]{2}[0-9]{4}/i)?.[0] || fullText;

        if (licensePlate) {
        console.log('Parsed License Plate:', licensePlate);

          navigation.navigate('AddCar', { licensePlate });
        } else {
          Alert.alert(
            'No Text Found',
            'Could not detect a license plate. Please try again.',
          );
        }
      } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomContainer}>
        ...
        <Text style={styles.bottomSubtitle}>
          Position the license plate in the frame
        </Text>
